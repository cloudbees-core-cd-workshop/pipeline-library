...
def call(imageName, imageTag) {
    ...
    podTemplate(name: 'kubectl', label: label, namespace: 'cb-deploy',  yaml: podYaml) {
      node(label) {
        ...
        container("kubectl") {
          sh("sed -i.bak 's#REPLACE_IMAGE_TAG#946759952272.dkr.ecr.us-east-1.amazonaws.com/${env.IMAGE_REPO}/${imageName}:${BUILD_NUMBER}#' deploy.yml")
          sh("sed -i.bak 's#REPLACE_SERVICE_NAME#${env.IMAGE_REPO}#' deploy.yml")
          sh "kubectl --namespace=cb-deploy apply -f deploy.yml"
          sh "echo 'deployed to https://prod.workshop.beedemo.net/${env.IMAGE_REPO}/'"
        }
      }
    }
}
