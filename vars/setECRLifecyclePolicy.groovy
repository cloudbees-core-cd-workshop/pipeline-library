  ...
  podTemplate(name: 'aws-cli', label: label, yaml: podYaml) {
    node(label) {
      container('aws-cli') {
        ...
        sh """aws ecr put-lifecycle-policy --region us-east-1 --repository-name ${name} --lifecycle-policy-text '$lifecyclePolicy'"""
        try {
          errorMsg = sh(returnStdout: true, script: "aws ecr create-repository --region us-east-1 --repository-name ${name} | tr -d '\n'")
        } catch(e) {
