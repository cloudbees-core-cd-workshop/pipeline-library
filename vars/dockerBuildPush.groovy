...
def call(String imageName, String imageTag = env.BUILD_NUMBER, String target = ".", String dockerFile="Dockerfile", Closure body) {
  ...
  setECRLifecyclePolicy("${env.IMAGE_REPO}/${imageName}")
  def label = "kaniko"
    ...
    node(label) {
      container(name: 'kaniko', shell: '/busybox/sh') {
        ...
        withEnv(['PATH+EXTRA=/busybox:/kaniko']) {
          sh """#!/busybox/sh
            executor -f ${pwd()}/${dockerFile} -c ${pwd()} --build-arg buildNumber=${BUILD_NUMBER} -d ${dockerReg}/${env.IMAGE_REPO}/${imageName}:${imageTag}
          """
        }
        }
      }
